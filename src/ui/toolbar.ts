...
export function initToolbar(container: HTMLElement): void {
  ...
  darkBtn.addEventListener('click', () => {
    ...
    localStorage.setItem('plotvision-theme', dark ? 'light' : 'dark');
  });
  container.appendChild(darkBtn);

  // Feedback
  const feedbackBtn = mkBtn(`<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>`, '', 'Send feedback');
  feedbackBtn.setAttribute('aria-label', 'Send feedback');
  feedbackBtn.addEventListener('click', () => {
    const url = 'https://github.com/Nisarg2543/PlotVision/issues/new?labels=feedback&title=Feedback%3A+';
  ...
  // Responsive hamburger: toggle right panel on narrow screens
  const hamburger = document.createElement('button');
  hamburger.className = 'tb-btn';
  hamburger.setAttribute('aria-label', 'Toggle sidebar');
  hamburger.innerHTML = `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="18" x2="21" y2="18"/></svg>`;
...
function buildShortcutsModal(): void {
  ...
  const shortcuts: [string, string][] = [
    ...
    ['M', 'Measure'],            ['E', 'Eraser'],
    ['B', 'Scale bar'],          ['P', 'Perspective correction'],
    ['R', 'Region of Interest'], ['Space+drag', 'Pan'],
